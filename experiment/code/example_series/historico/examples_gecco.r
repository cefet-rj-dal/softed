...
score_data$is.anomaly <- score_data$timestamp %in% events_scp$time
...
#Detect
events_mdl_outlier <- evtdet.mdl_outlier(test,mdl=garch11,na.action=na.omit,spec=garch11)
#Evaluate
evaluate(events_mdl_outlier, reference, metric="confusion_matrix")
#Plot
print(evtplot(test,events_mdl_outlier, reference))
...
evaluate(events_garch_volatility_outlier, reference, metric="precision")
evaluate(events_garch_volatility_outlier, reference, metric="recall")
evaluate(events_garch_volatility_outlier, reference, metric="F1")

soft_evaluate(events_garch_volatility_outlier, reference, metric="precision")
soft_evaluate(events_garch_volatility_outlier, reference, metric="recall")
soft_evaluate(events_garch_volatility_outlier, reference, metric="F1")

#==== NAB score ====
score_data <- reference
colnames(score_data) <- c("timestamp","is.real.anomaly")
score_data$is.anomaly <- score_data$timestamp %in% events_garch_volatility_outlier$time
score_data$value <- test[,2]
otsad::GetDetectorScore(score_data, print = TRUE, title = "")