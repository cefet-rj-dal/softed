...
evt_exp_analysis <- function(data_results, dataset_name){
  #vh,vs: vs-vh (diffs; % of diffs > 0; % of diffs == 0)
  
  diff_dataset <- lapply(data_results, function(dataset) sapply(dataset$metric_var, function(serie) serie$rival[-1]-serie$hard[-1] ))
  diff_dataset <- as.numeric(as.matrix(unlist(diff_dataset)))
  
  diff_data <- diff_dataset[!is.na(diff_dataset)]
  
  percH0_diff <- length(diff_dataset[!is.na(diff_dataset) & diff_dataset>0])/length(diff_dataset[!is.na(diff_dataset) | is.nan(unlist(diff_dataset))]) 
  percE0_diff <- length(diff_dataset[!is.na(diff_dataset) & diff_dataset==0])/length(diff_dataset[!is.na(diff_dataset) | is.nan(unlist(diff_dataset))])
  
  #NA,NA;NA,vs: vs-vh = NA (% of NA results)
  NA_diff <- length(diff_dataset[is.na(diff_dataset) & !is.nan(unlist(diff_dataset))])
  
  #NaN,NaN;NaN,vs: vs (soft F1s; % of Soft F1>=0 and hard F1 N/A)
  NaNH_diff_dataset <- lapply(data_results, function(dataset) sapply(dataset$metric_var, function(serie) serie$rival[-1][!is.na(serie$rival[-1]) & is.nan(unlist(serie$hard[-1]))] ))
  NaNH_diff_dataset <- as.numeric(unlist( NaNH_diff_dataset ))

  diffNaN <- NaNH_diff_dataset
  
  percNaNH_diff <- length( NaNH_diff_dataset )/length(as.matrix( diff_dataset[!is.na(diff_dataset) | is.nan(unlist(diff_dataset))]))
  
  NaN2_diff_dataset <- lapply(data_results, function(dataset) sapply(dataset$metric_var, function(serie) serie$rival[-1][is.nan(unlist(serie$rival[-1])) & is.nan(unlist(serie$hard[-1]))] ))
  NaN2_diff_dataset <- as.numeric(unlist(NaN2_diff_dataset))
  percNaN2_diff <- length( NaN2_diff_dataset )/length(as.matrix( diff_dataset[!is.na(diff_dataset) | is.nan(unlist(diff_dataset))]))
  ...
  diffTop <- lapply(data_results, function(dataset) {
    diff_dataset <- sapply(dataset$metric_var, function(serie) {
      ...
      top5_change <- sapply(names(rank_rival), function(method) match(method,names(rank_hard))-match(method,names(rank_rival)) )[1:5]
      top5_change[top5_change>0]
    } )
  }
  )
  diffTop <- as.numeric(unlist( diff_dataset ))
  browser()
  diff_dataset_top5 <- lapply(data_results, function(dataset) sapply(dataset$metric_var, function(serie) (serie$rival[-1]-serie$hard[-1])[1:5] ))
  diff_dataset_top5 <- as.matrix(unlist(diff_dataset_top5))
  
  percNewTop_diff <- lapply(data_results, function(dataset) {
    sapply(dataset$metric_var, function(serie) {
      rank_rival <- serie$rival[-1][order(t(serie$rival[-1]),decreasing = TRUE)]
      rank_hard <- serie$hard[-1][order(t(serie$hard[-1]),decreasing = TRUE)]
      top5_change <- sapply(names(rank_rival), function(method) match(method,names(rank_hard))-match(method,names(rank_rival)) )[1:5]
      !(names(rank_rival)[1:5][top5_change>0] %in% names(rank_hard)[1:5])
    } )
  }
  )
  percNewTop_diff <- sum(as.numeric(unlist( percNewTop_diff )))/length(diff_dataset_top5)
  ...
  percTop_diff <- sum(as.numeric(unlist( percTop_diff )))/length(diff_dataset_top5) 
  ...
  percSameTop_diff <- length(as.numeric(unlist( percSameTop_diff )))/length(diff_dataset_top5)
  ...
  percLowerTop_diff <- length(as.numeric(unlist( percLowerTop_diff )))/length(diff_dataset_top5) 
  
  #Boxplot of the differences between Soft F1 and Hard F1
  diff_plot <- cbind(L1=dataset_name,reshape2::melt(diff_data))
...
evt_exp_analysis_pr <- function(data_results){
  ...
  percNewTop_diff <- lapply(data_results, function(dataset) {
    ...
    diff_dataset_total <- sapply(dataset$metric_var, function(serie) (serie$rival[-1]-serie$hard[-1])[1:5] )
...
evt_exp_analysis_3w <- function(data_results){
  ...
  percNewTop_diff <- lapply(data_results, function(dataset) {
    ...
    diff_dataset_total <- sapply(dataset$metric_well, function(well) sapply(well, function(serie) (serie$rival[-1]-serie$hard[-1])[1:5] ))
    sum(as.numeric(unlist( diff_dataset )))/length(as.matrix( diff_dataset_total)) 
  }
  )
  percTop_diff <- lapply(data_results, function(dataset) {
...
k_evt_exp_analysis <- function(results){
...
k_evt_exp_analysis_3w <- function(results){
  k_results <- list()
  for(k in names(results)){
...
all_evt_exp_analysis <- function(results){
  ...
  return(list(F1=evt_exp_analysis(results$F1),
              precision=evt_exp_analysis_pr(results$precision),
              recall=evt_exp_analysis_pr(results$recall)))
}

all_evt_exp_analysis_3w <- function(results){
  
  return(list(F1=evt_exp_analysis_3w(results$F1),
              precision=evt_exp_analysis_pr_3w(results$precision),
              recall=evt_exp_analysis_pr_3w(results$recall)))
}
...
plot_evt_exp_analysis <- function(evt_exp_analysis,dataset_name, rival=c("SoftED","NAB")){
  ...
  file_name <- paste0("plots/",rival_metric,"vsHard/",dataset_name,"/climb_top5_",rival_metric,".pdf")
  CairoPDF(file_name,width=7,height=6)
  print(p)
  dev.off()
  ...
  file_name <- paste0("plots/",rival_metric,"vsHard/",dataset_name,"/analysis_top5_",rival_metric,".pdf")
...
plot_evt_exp_comparative <- function(evt_exp_analysis_soft,evt_exp_analysis_nab,dataset_name){
  ...
  file_name <- paste0("plots/comparative/",dataset_name,"/analysis_top5.pdf")
  CairoPDF(file_name,width=10,height=6)
  print(p)
  dev.off()
}
...
plot_evt_exp_analysis_by_k <- function(evt_exp_analysis,dataset_name){
  ...
  file_name <- paste0("plots/SoftEDbyK/",dataset_name,"/climb_top5_SoftED.pdf")
  CairoPDF(file_name,width=7,height=6)
  print(p)
  dev.off()
  ...
  file_name <- paste0("plots/SoftEDbyK/",dataset_name,"/analysis_top5.pdf")
  CairoPDF(file_name,width=10,height=6)
  print(p)
  dev.off()
}